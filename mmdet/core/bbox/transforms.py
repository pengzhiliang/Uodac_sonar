...
def bbox2delta_xyxy(proposals, gt, means=[0, 0, 0, 0], stds=[1, 1, 1, 1]):
    assert proposals.size() == gt.size()
    ...
    dx1 = (gt[..., 0] - proposals[..., 0]) / pw
    dy1 = (gt[..., 1] - proposals[..., 1]) / ph
    dx2 = (gt[..., 2] - proposals[..., 2]) / pw
    dy2 = (gt[..., 3] - proposals[..., 3]) / ph
...
def delta_xyxy2bbox(rois,
    ...
    # Convert center-xy/width/height to top-left, bottom-right
    x1 = dx1 * pw + rois[:, 0].unsqueeze(1).expand_as(dx1)
    y1 = dy1 * ph + rois[:, 1].unsqueeze(1).expand_as(dy1)
    x2 = dx2 * pw + rois[:, 2].unsqueeze(1).expand_as(dx2)
    y2 = dy2 * ph + rois[:, 3].unsqueeze(1).expand_as(dy2)
    if max_shape is not None:
        x1 = x1.clamp(min=0, max=max_shape[1] - 1)
        y1 = y1.clamp(min=0, max=max_shape[0] - 1)
        x2 = x2.clamp(min=0, max=max_shape[1] - 1)
...
def bbox_flip(bboxes, img_shape, flip_direction='horizontal'):
...
def bbox2result(bboxes, labels, num_classes, with_variance=False):
    ...
    if not with_variance:
        ...
        # [vx1, vy1, vx2, vy2, x1, y1, x2, y2, score]
        if bboxes.shape[0] == 0:
            return [
