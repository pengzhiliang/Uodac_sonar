...
def anchor_target_single(flat_anchors,
    ...
    if len(pos_inds) > 0:
        ...
        if 'gt_bboxes_weight' in img_meta:
            bbox_weights[pos_inds, :] = img_meta['gt_bboxes_weight'][sampling_result.pos_assigned_gt_inds][:, None].to(label_weights.device)
        else:
        ...
        if cfg.pos_weight <= 0:
            if 'gt_bboxes_weight' in img_meta:
                label_weights[pos_inds] = img_meta['gt_bboxes_weight'][sampling_result.pos_assigned_gt_inds].to(label_weights.device)
            else:
