...
def soft_nms_variance_voting(dets, variance, iou_thr, method='linear', sigma=0.5, sigma_t=0.02, min_score=1e-3,
                             merge=False):
    ...
    if isinstance(dets, torch.Tensor):
        is_tensor = True
        dets_t = dets.detach().cpu()
        variance_t = variance.detach().cpu().detach().cpu()
    elif isinstance(dets, np.ndarray):
        is_tensor = False
        dets_t = torch.from_numpy(dets)
        variance_t = torch.from_numpy(variance)
    else:
        raise TypeError(
            'dets must be either a Tensor or numpy array, but got {}'.format(
                type(dets)))
    ...
    is_merge = 1 if merge else 0

    results = nms_cpu.soft_nms_variance_voting(dets_t, variance_t, iou_thr, sigma_t,
                                               method_codes[method], sigma, min_score, is_merge)
