...
def main():
    ...
    if args.cross_validate:
        _, original_output = os.path.split(args.out)
        root_dir, original_checkpoint = os.path.split(args.checkpoint)
        original_ann = cfg.data.test.ann_file
        for i in range(4):
            now_output = osp.join(root_dir, '{}_split'.format(str(i + 1)), original_output)
            now_checkpoint = osp.join(root_dir, '{}_split'.format(str(i + 1)), original_checkpoint)
            now_ann = original_ann.replace('re_', '{}_split_'.format(str(i + 1)))

            args.out = now_output
            args.checkpoint = now_checkpoint
            cfg.data.test.ann_file = now_ann
